...
DCLog *NewDCLog( void ) {
  ...
  if ((dclog = (DCLog*)malloc( sizeof( DCLog ) )) == NULL) {

    fprintf( stderr, "NewDCLog() could not malloc() %ld bytes, is the system "
             "low on memory?\n", sizeof( DCLog ) );
  ...
  dclog->mailhost = (char*) malloc( (strlen( DCLOG_MAILHOST ) + 1) * sizeof( char ) );
  strcpy( dclog->mailhost, DCLOG_MAILHOST );
  dclog->to = (char*) malloc( (strlen( DCLOG_TO ) + 1) * sizeof( char ) );
...
char DCLogLevelToNum( const char *str ) {
  ...
  for (i = 0; levels[(int)i] != NULL; i++)
    if (strncmp( str, levels[(int)i], 10 ) == 0)
      return i;
...
UCHAR DCLogSetMailServer( DCLog *dclog, const char *mailhost ) {
  ...
  if ((dclog->mailhost = (char*) malloc( (strlen( mailhost ) + 1) * sizeof( char ) ))
      == NULL)
    return 0;
...
UCHAR DCLogSetPageServer( DCLog *dclog, const char *server ) {
  ...
  if (dclog->to != NULL) {
    ...
    // Allocate enough memory for dclog->to and set it
  if ((dclog->to = (char*) malloc( (strlen( server ) + 1) * sizeof( char ) ))
      == NULL)
    return 0;
...
UCHAR DCLogSetProgram( DCLog *dclog, const char *prog ) {
  ...
  printf("\n\n\n\nSee in DCLogSetProgram\n\n\n\n");
  printf("\n\n\n\nlook for gethostname\n\n\n\n");
  abort();
  ...
  from = (char*) malloc( (strlen( prog ) + strlen( hostname ) + 2)
                 * sizeof( char ) );
...
UCHAR DCLogOpen( DCLog *dclog, const char *fn, const char *mode ) {
  ...
  if (dclog->fn == NULL &&
      (dclog->fn = (char*) malloc( sizeof( char ) * DCLOG_FN_LEN )) == NULL)
    return 0;
...
UCHAR DCLogClose( DCLog *dclog ) {
  ...
  if (strcmp( dclog->fn, "stderr" ) == 0 ||
      strcmp( dclog->fn, "stdout" ) == 0)
    retval = 0;
  else
...
inline UCHAR DCLogWrite( DCLog *dclog, const UCHAR lev,
                         const char *fmt, va_list str_args ) {
...
//      localtime_r( &current_time, dclog->prev_date );
      dclog->prev_date = localtime( &current_time );
