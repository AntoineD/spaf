// this is a modification of the original ANN library in order to have the search report points that are outside the bounding box (Antoine)
#define ANN_REPORT_POINT_OUTSIDE_BOUNDING_BOX
...
//----------------------------------------------------------------------
//	annIsInsideBoundingBox - test if a point is inside athe bounding box
//----------------------------------------------------------------------

bool ANNkd_tree::annIsInsideBoundingBox(
ANNpoint  point ) // the query point
{  
  return annBoxDistance(point, bnd_box_lo, bnd_box_hi, dim) > 0.;
}
...
#ifdef ANN_REPORT_POINT_OUTSIDE_BOUNDING_BOX
  if ( box_dist > 0. )
  {
